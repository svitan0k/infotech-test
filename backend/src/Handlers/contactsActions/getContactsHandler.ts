...
//SELECT username FROM users INNER JOIN addedContacts ON addedContacts.contact = users.id WHERE addedContacts.owner = (SELECT id FROM users WHERE username = 'Ivan') AND NOT username IN (SELECT username FROM users INNER JOIN blockedContacts ON blockedContacts.contact = users.id WHERE blockedContacts.owner = (SELECT id FROM users WHERE username = 'Ivan')); 
